  ...
  return (
    <div className="space-y-5">
      <header className="glass rounded-3xl p-6">
        ...
        <h1 className="text-3xl font-bold text-slate-100">Cómo funciona</h1>
        <p className="mt-2 text-slate-300">Encuentra al profesional ideal en 3 pasos.</p>
      </header>
      <div className="grid gap-4 md:grid-cols-3">
        {["Busca por oficio y ubicación", "Compara valoraciones y tarifas", "Solicita y recibe presupuesto"].map((t, i) => (
          <article key={t} className="glass rounded-2xl p-5">
            <p className="mb-2 text-xs text-slate-400">Paso {i + 1}</p>
            <h2 className="font-semibold text-slate-100">{t}</h2>
          </article>
        ))}
      </div>
    </div>
