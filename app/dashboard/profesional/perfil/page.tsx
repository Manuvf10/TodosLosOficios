...
export default function PerfilProfesionalPage() {
  ...
  const { register, handleSubmit } = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: { description: profile?.description || "", baseRate: profile?.baseRate || 30, city: profile?.city || "" },
  });

  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
      <Card>
        <h1 className="mb-4 text-2xl font-bold text-slate-100">Editar perfil profesional</h1>
        <form
      ...
      <Card>
        <h2 className="mb-2 text-lg font-semibold text-slate-100">Preview</h2>
        <p className="text-sm text-slate-300">Así verán tu perfil los clientes. Mantén descripción clara y precio competitivo.</p>
      </Card>
    </div>
  );
}
